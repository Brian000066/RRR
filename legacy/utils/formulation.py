...
class FormulationConfig:
    ...
    bandwidth_time_fraction: float = 0.2
    min_bandwidth: float = 0.0
    default_channel_gain: float = 1.0
    default_power: float = 1.0
    common_power_ratio: float = 0.4
    beamforming_correlation_weight: float = 1.0
    noise_power: float = 8.004e-14
    wavelength: float = 0.125
    max_group_size: int = 4
    min_rate: float = 1.0
    output_token_bits: float = 16.0
    default_feature_bits: float = 8_000.0
...
class FormulationEvaluator:
    ...
    def evaluate(
        ...
        timing = self.compute_timing(assignments, groups, backhaul, common_rates, private_rates)
        violations = self.check_constraints(assignments, groups, timing, common_rates, private_rates)
        ...
        for task in self.tasks.values():
            ...
            while remaining:
                ...
                for sub in ready:
                    key = (task.id, sub.id)
                    for server_id in self.participating_servers(assignments, key):
                        feature_ready[(key, server_id)] = self.feature_ready_time(server_id, sub, group_by_device, uplink, backhaul)
    ...
    def check_constraints(
        ...
        for task in self.tasks.values():
            for sub in task.subtasks:
                ...
                loss = sub.max_loss if sub.max_loss is not None else self.estimated_loss((task.id, sub.id), assignments)
                if loss > threshold:
        ...
        for group in groups:
            ...
            if common < self.config.min_rate:
    ...
    def feature_ready_time(
        ...
        if not subtask.required_features:
            return 0.0
        ready = 0.0
        for feature in subtask.required_features:
            best = float("inf")
    ...
    def performance_loss_from_probability(
        ...
        return (1.0 / probability) + (
            self.config.lambda_reconstruction * subtask.reconstruction_loss
        )
    ...
    def required_selection_probability(self, subtask: SubtaskSpec) -> float:
        threshold = self.conformal_loss_threshold(subtask)
        if threshold == float("inf"):
            return 0.0
        denominator = threshold - (self.config.lambda_reconstruction * subtask.reconstruction_loss)
        if denominator <= 0.0:
            return float("inf")
    ...
    def estimated_loss(
        ...
        return self.performance_loss_from_probability(subtask, probability)
